...
@section('content')
@if(empty($url))
    <input type="hidden" id="page_url" value="{{ route('team.discrepancies') }}">
@else
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12">
                                <select class="select2 form-select" id="redirectDropdown" onchange="redirectPage(this)">
                                    <option value="{{ URL::to('team/discrepancies') }}">All Team Discrepancies</option>
                                    @foreach ($employees as $employee)
                                        <option value="{{ URL::to('team/discrepancies/'.$employee->slug) }}" {{ $user->slug==$employee->slug?"selected":"" }} >{{ $employee->first_name }} {{ $employee->last_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
