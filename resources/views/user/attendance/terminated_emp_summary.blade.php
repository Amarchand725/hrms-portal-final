            ...
            <div class="card">
                        ...
                        <span class="card-title mb-0">
                            <a href="{{ route('employees.show', $data['user']->slug) }}" class="text-body text-truncate">
                                ...
                                @if(isset($data['user']->profile) && !empty($data['user']->profile->profile))
                                    <img src="{{ asset('public/admin/assets/img/avatars') }}/{{ $data['user']->profile->profile }}" style="width:40px !important; height:40px !important" alt class="h-auto rounded-circle" />
                                @else
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12">
                                <select class="select2 form-select" id="redirectDropdown" onchange="redirectPage(this)">
                                    <option value="" selected>Select terminated employee</option>
                                    @foreach ($data['employees'] as $employee)
                                        @if(isset($data['year']) && !empty($data['year']))
                                            <option value="{{ URL::to('user/attendance/terminated_employee_summary/'.$data['month'].'/'.$data['year'].'/'.$employee->slug) }}" {{ $data['user']->slug==$employee->slug?'selected':'' }}>{{ $employee->first_name }} {{ $employee->last_name }}</option>
                                        @else
                                            @php
                                                $data['month'] = date('m', strtotime($employee->employeeStatus->end_date));
                                                $data['year'] = date('Y', strtotime($employee->employeeStatus->end_date));
                                            @endphp
                                            <option value="{{ URL::to('user/attendance/terminated_employee_summary/'.$data['month'].'/'.$data['year'].'/'.$employee->slug) }}">{{ $employee->first_name }} {{ $employee->last_name }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                ...
                @if(isset($data['user_slug']) && !empty($data['user_slug']))
                    ...
                    @php
                        ...
                        $currentMonthStart = Carbon::create($data['year'], $data['month'], 5, 21, 0, 0);
                        $currentMonthEnd = Carbon::create($data['year'], $data['month'], 26, 6, 0, 0);
                            ...
                            @for($i = $begin; $i <= $end; $i->modify('+1 day'))
                                ...
                                @if($reponse!=null)
                                    ...
                                    @if(!empty($reponse['attendance_id']))
                                        @php
                                            $attendance_adjustment = attendanceAdjustment($data['user']->id, $reponse['attendance_id']);
                                        @endphp
                                    @endif
                                    <tr class="{{$day}}">
